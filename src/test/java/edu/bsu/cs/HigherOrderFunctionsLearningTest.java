...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testMap() {
        ...
        List<Integer> actual = input.map(n->n+1).collect(Collectors.toList()); // TODO: Replace the `actual` with your solution, here and in all the following.
    ...
    public void testFilter() {
        ...
        List<String> actual = input.filter(entry -> entry.startsWith("b")).collect(Collectors.toList());
    ...
    public void testDistinct() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        List<String> actual = input.map(entry -> entry=entry.substring(0,1)).distinct().collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(entry -> Integer.parseInt(entry.substring(1,2)) %2 ==0).collect(Collectors.joining());
    ...
    public void testCountWhitelisted() {
        ...
        long actual = input.filter(revision -> whitelist.contains(revision.user)).collect(Collectors.toList()).stream().count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(revision -> !revision.user.toUpperCase().contains("BOT")).collect(Collectors.toList()).stream().count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesInFebruary() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(revision -> revision.timestamp.toString().contains("-02-")).count();
        int expected = 9;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesByMonth() {
        Stream<Revision> input = getRevisions("soup04.json");
        Map<Month, Long> actual = input.map(revision -> revision.timestamp.atZone(ZoneId.systemDefault()).getMonth())
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = input.map(revision -> revision.timestamp.atZone(ZoneId.systemDefault()).getMonth())
                .collect(Collectors.groupingBy(m -> m, Collectors.counting())).entrySet().stream().max(Map.Entry.comparingByValue()).get().getKey();
